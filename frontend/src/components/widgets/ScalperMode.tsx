  ...
  return (
    <div className="card-glass rounded-lg overflow-hidden">
      ...
      <div className="px-3 py-2 border-b border-[var(--color-border-primary)] flex items-center gap-3">
        <Crosshair className="w-4 h-4 text-[var(--color-neon-cyan)]" />
        <h3 className="text-sm font-semibold text-[var(--color-text-muted)] uppercase tracking-wider">
          Scalper Mode
        </h3>
        <span className="text-[11px] font-mono px-1.5 py-0.5 rounded uppercase font-bold text-[var(--color-neon-cyan)]"
          style={{
      ...
      <div className="flex border-b border-[var(--color-border-primary)]">
        {([
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1 py-2 min-h-[36px] lg:min-h-0 text-[12px] font-semibold uppercase tracking-wider transition-colors ${
              tab === t.key
      ...
      <div className="flex items-center gap-2">
        {TF_OPTIONS.map(tf => (
          <button
            ...
            className={`px-2 py-0.5 rounded text-[12px] font-bold uppercase transition-colors ${
              selectedTf === tf
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="text-sm font-bold uppercase" style={{ color: dirColor }}>
          {isLong ? "BUY" : "SELL"}
        </span>
        <span className="text-[11px] font-mono px-1 py-0.5 rounded uppercase"
          style={{
            color: status.text,
        ...
        {signal.mtf_confluence && (
          <span className="text-[10px] font-mono px-1 py-0.5 rounded uppercase font-bold"
            style={{
        ...
        <span className="text-[11px] font-mono px-1 py-0.5 rounded bg-[var(--color-bg-hover)] text-[var(--color-text-muted)]">
          {signal.timeframe}
        </span>

        <span className="ml-auto text-[15px] font-bold font-mono" style={{ color: dirColor }}>
      ...
      <div className="grid grid-cols-3 gap-1 mb-1">
        <div className="text-center">
          ...
          <div className="text-[13px] font-mono font-bold text-[var(--color-text-primary)]">
            {signal.entry_price.toLocaleString()}
          </div>
        </div>
        <div className="text-center">
          <div className="text-[10px] text-[var(--color-bear)] uppercase flex items-center justify-center gap-0.5">
            <Shield className="w-2 h-2" /> SL
          </div>
          <div className="text-[13px] font-mono font-bold text-[var(--color-bear)]">
            {signal.stop_loss.toLocaleString()}
          </div>
        </div>
        <div className="text-center">
          <div className="text-[10px] text-[var(--color-bull)] uppercase flex items-center justify-center gap-0.5">
            <Target className="w-2 h-2" /> TP
          </div>
          <div className="text-[13px] font-mono font-bold text-[var(--color-bull)]">
        ...
        <div className="flex gap-1">
          {Object.entries(analytics.by_timeframe).map(([tf, stats]: [string, any]) => (
            <div key={tf} className="flex-1 rounded-md bg-[var(--color-bg-hover)] p-1.5 text-center">
              <div className="text-[11px] font-bold text-[var(--color-text-muted)] uppercase">{tf}</div>
              <div className="text-[13px] font-mono" style={{ color: stats.win_rate > 50 ? "var(--color-bull)" : "var(--color-bear)" }}>
      ...
      {journal?.entries && journal.entries.length > 0 && (
        <div className="space-y-1">
          {journal.entries.slice(0, 8).map((entry, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-2 px-1.5 py-1 rounded-md bg-[var(--color-bg-secondary)] border border-[var(--color-border-primary)]">
                <StatusIcon className="w-4 h-4 shrink-0" style={{ color: statusColor }} />
                <span className="text-[12px] font-bold uppercase" style={{ color: entry.direction === "long" ? "var(--color-bull)" : "var(--color-bear)" }}>
                  {entry.direction === "long" ? "BUY" : "SELL"}
                </span>
                <span className="text-[11px] font-mono text-[var(--color-text-muted)]">{entry.timeframe}</span>
...
function LearningTab({ data }: { data: LossData | null }) {
  ...
  return (
    <div className="space-y-2">
      {/* Win rate comparison */}
      <div className="rounded-md bg-[var(--color-bg-secondary)] p-3 border border-[var(--color-border-primary)]">
        <div className="flex items-center justify-between mb-1">
          <span className="text-[12px] text-[var(--color-text-muted)] uppercase font-semibold">Win Rate Impact</span>
          <span className="text-[11px] font-mono text-[var(--color-neon-green)]">
            {data.active_filters} filter{data.active_filters !== 1 ? "s" : ""} active
          </span>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-center flex-1">
            <div className="text-[10px] text-[var(--color-text-muted)] uppercase">Current</div>
            <div className="text-[15px] font-bold font-mono" style={{
              color: data.win_rate > 50 ? "var(--color-bull)" : "var(--color-bear)",
            }}>
              {data.win_rate}%
            </div>
          </div>
          <ChevronRight className="w-4 h-4 text-[var(--color-neon-green)]" />
          <div className="text-center flex-1">
            <div className="text-[10px] text-[var(--color-text-muted)] uppercase">With Filters</div>
            <div className="text-[15px] font-bold font-mono text-[var(--color-neon-green)]">
              {data.adjusted_win_rate}%
            </div>
          </div>
          {data.improvement > 0 && (
            <div className="text-center">
              <div className="text-[10px] text-[var(--color-text-muted)] uppercase">Improvement</div>
              <div className="text-[15px] font-bold font-mono text-[var(--color-neon-green)]">
                +{data.improvement}%
              </div>
            </div>
      ...
      {data.loss_breakdown && Object.keys(data.loss_breakdown).length > 0 && (
        <div>
          <div className="text-[12px] text-[var(--color-text-muted)] uppercase font-semibold mb-1 flex items-center gap-1">
            ...
            {Object.entries(data.loss_breakdown)
              ...
              .map(([cat, stats]: [string, any]) => (
                <div key={cat} className="flex items-center gap-1">
                  <span className="text-[12px] w-3 text-center">{LOSS_CATEGORY_ICONS[cat]}</span>
                  <span className="text-[11px] font-mono w-20 truncate text-[var(--color-text-muted)]">
                    {cat.replace(/_/g, " ")}
                  </span>
                  ...
                  <span className="text-[11px] font-mono w-6 text-right text-[var(--color-bear)]">
                    {stats.count}
                  </span>
                </div>
              ))}
            ...
            {data.patterns.map(p => (
              ...
              >
                <div className="flex items-center gap-1 mb-0.5">
                  <span className="text-[12px]">{LOSS_CATEGORY_ICONS[p.category]}</span>
                  <span className="text-[12px] font-bold uppercase text-[var(--color-neon-amber)]">
                    {p.category.replace(/_/g, " ")}
                  </span>
                  <span className="ml-auto text-[11px] font-mono text-[var(--color-bear)]">
