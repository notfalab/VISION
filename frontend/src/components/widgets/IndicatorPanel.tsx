  ...
  return (
    <div className="rounded-md bg-[var(--color-bg-secondary)] px-2.5 py-2 border border-[var(--color-border-primary)] hover:border-[var(--color-border-secondary)] transition-colors">
      <div className="flex items-center gap-1.5 mb-1">
        <Icon className="w-3 h-3 shrink-0" style={{ color: signalColor }} />
        <span className="text-[10px] font-semibold text-[var(--color-text-primary)] uppercase tracking-wide">
          {config?.title || ind.name}
        </span>
        <span
          className="text-[8px] font-mono px-1 py-0.5 rounded ml-auto uppercase tracking-wide"
          style={{
            color: signalColor,
      ...
      <div className="flex items-baseline gap-2">
        <span className="text-xs font-mono font-bold" style={{ color: signalColor }}>
          {desc.value}
        </span>
      </div>
      <p className="text-[9px] text-[var(--color-text-muted)] leading-relaxed mt-0.5">{desc.description}</p>
    </div>
  );
}
          ...
          <div className="space-y-2">
            {(Object.entries(categories) as [string, IndicatorData[]][]).map(
              ([cat, items]) =>
                items.length > 0 && (
                  <div key={cat}>
                    <div className="text-[8px] text-[var(--color-text-muted)] uppercase tracking-widest font-semibold mb-1 px-0.5">
                      {CATEGORY_LABELS[cat]}
                    </div>
