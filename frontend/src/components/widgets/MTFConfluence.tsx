      ...
      <div className="p-2">
        {loading ? (
          <div className="flex items-center justify-center gap-2 py-4">
            ...
            <span className="text-[10px] text-[var(--color-text-muted)]">Analyzing timeframes...</span>
          </div>
        ) : !data ? (
                ...
                <div key={ind} className="grid grid-cols-[1fr_36px_36px_36px] md:grid-cols-[1fr_40px_40px_40px] gap-0 px-2 py-1 border-b border-[var(--color-border-primary)] last:border-b-0">
                  ...
                  {["1h", "4h", "1d"].map(tf => {
                    ...
                    if (!tfData?.available || !tfData.indicators[ind]) {
                      return <span key={tf} className="text-center text-[8px] text-[var(--color-text-muted)]">—</span>;
                    }
                ...
                {Object.entries(data.confluence).map(([ind, conf]) => {
                  ...
                  return (
                    <div key={ind} className="flex items-center gap-1.5">
                      <span className="text-[7px] font-mono text-[var(--color-text-muted)] w-14 shrink-0">
                        {IND_LABELS[ind] || ind}
                      </span>
                      ...
                      <span className="text-[7px] font-mono w-6 text-right" style={{ color }}>
                        {conf.score}%
                      </span>
                    </div>
                  );
...
function SignalDot({ signal }: { signal: string }) {
  if (signal === "bullish") return <TrendingUp className="w-3 h-3 text-[var(--color-bull)]" />;
  if (signal === "bearish") return <TrendingDown className="w-3 h-3 text-[var(--color-bear)]" />;
